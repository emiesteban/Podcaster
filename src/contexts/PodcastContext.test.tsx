...
describe('PodcastProvider', () => {
  ...
  test('should update the podcastList when setPodcastList is called', () => {
    const { getByText } = render(
      <PodcastProvider>
        <PodcastContext.Consumer>
          {({ podcastList, setPodcastList }) => (
            <>
            <h1 data-testid="artist">{podcastList?.list[0]?.artist}</h1>
            <button onClick={() => setPodcastList({ timestamp: 1, list: [{ id: 1, artist: 'Test Podcast' }] })}>
              Update Podcast List
            </button>
            </>
          )}
    ...
    waitFor(() => expect(getByText('Test Podcast')).toBeInTheDocument());
